...
function getAgent() {
  return getExpertAgent('content_media');
}
...
router.post('/optimize-image', async (req, res) => {
  try {
    ...
    const optimization = await agent.execute('optimizeImage', { path, quality });
...
router.post('/process-video', async (req, res) => {
  try {
    ...
    const processing = await agent.execute('processVideo', { path, targetFormat });
    res.json(processing);
  } catch (error) {
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const stats = await agent.execute('getMediaStats', {});
    res.json(stats);
  } catch (error) {
...
router.post('/suggest-format', async (req, res) => {
  try {
    ...
    const suggestion = await agent.execute('suggestFormat', { type, usage });
    res.json(suggestion);
  } catch (error) {
