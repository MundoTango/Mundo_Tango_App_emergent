...
- **ESA MindMap (Context-Aware Floating Navigator)**: Globally accessible floating button in admin areas with magnetic interactions. Provides Quick Navigator Overlay featuring live stats (105 agents, 61 layers, training progress), instant agent search, and context-aware page detection showing which agents built the current page (e.g., "Agent #65, #64, #6 built this page"). Includes 5 quick actions (Full Dashboard, Org Chart, Quality Gates, 17-Phase Audit, Training Status), agent hierarchy preview, and dynamic "Current Page Agents" section that detects route and displays responsible agents with primary builder highlighted. Built with Aurora Tide glassmorphic design and MT Ocean gradients. Uses agent-to-page registry (client/src/config/esaAgentPageRegistry.ts) for context detection.
