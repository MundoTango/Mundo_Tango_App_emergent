  ...
  if (compact) {
    return (
      <div className={`relative ${className}`}>
        ...
        {showShareMenu && (
          <div className="absolute top-full left-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 p-4 z-50 min-w-64 dark:bg-neutral-900">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {shareButtons.map(({ component: ShareButton, icon: Icon, label, props }) => (
                <ShareButton key={label} {...props} className="group">
                  <div className="flex flex-col items-center space-y-1 p-2 rounded-lg hover:bg-gray-50 transition-colors dark:bg-neutral-800">
                    <Icon size={24} round />
                    <span className="text-xs text-gray-600 group-hover:text-gray-800 dark:text-neutral-200">{label}</span>
                  </div>
                </ShareButton>
              ))}
            </div>
