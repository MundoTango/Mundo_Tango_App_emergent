...
export default function SocialShareButtons({
  ...
  if (compact) {
    return (
      <div className={`relative ${className}`}>
        ...
        {showShareMenu &&
        <div className="absolute top-full left-0 mt-2 bg-[var(--color-surface)] dark:bg-gray-900 rounded-lg shadow-lg border border-[var(--color-border)] p-4 z-50 min-w-64">
            ...
            <ShareButton key={label} {...props} className="group">
                  <div className="flex flex-col items-center space-y-1 p-2 rounded-lg hover:bg-[var(--color-surface-elevated)] transition-colors">
                    <Icon size={24} round />
                    <span className="text-xs text-gray-600 group-hover:text-gray-800 dark:text-gray-100">{label}</span>
                  </div>
                </ShareButton>
            )}
            </div>
            ...
            <div className="border-t border-[var(--color-border)] pt-3">
              <div className="flex items-center space-x-2">
                ...
                value={shareUrl}
                readOnly
                className="flex-1 px-3 py-2 bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg text-sm text-gray-600 dark:text-gray-300" data-testid="input-text" />

                <button
                onClick={copyToClipboard}
                className="flex items-center space-x-1 px-3 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition-colors" data-testid="button-flex" aria-label="Button">
  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      {showLabels &&
      <span className="text-sm font-medium text-[var(--color-text-secondary)] mr-2">Share:</span>
      }
      ...
      <button
        onClick={copyToClipboard}
        className="group relative p-1 text-gray-600 hover:text-gray-800 dark:text-gray-100 transition-colors"
        title="Copy link" data-testid="button-group" aria-label="Button">

        <div className="p-1 rounded-full bg-[var(--color-neutral-100)] hover:bg-gray-200 dark:bg-gray-700 transition-colors">
...
}: {postId: number;title: string;description?: string;hashtags?: string[];}) {
  return (
    <SocialShareButtons
