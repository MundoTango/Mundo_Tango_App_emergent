...
export const PostContextMenu: React.FC<PostContextMenuProps> = ({
  ...
  const nonCreatorActions = [
    ...
    },
    {
      ...
      className: 'text-gray-600 dark:text-gray-300 hover:bg-[var(--color-surface-elevated)]'
    }
  ];
    ...
    <div className={`relative ${className}`}>
      ...
      {isOpen && (
        <>
          ...
          {/* Menu */}
          <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--color-surface)] rounded-xl shadow-lg border border-[var(--color-border)] py-2 z-50 dark:bg-neutral-900">
            {actions.map((action, index) => (
              <button
                key={index}
                onClick={() = aria-label="Button"> action.action && handleMenuClick(action.action)}
                className={`
